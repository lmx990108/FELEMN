...
def min_corr_distribution(shard_num, dataset="datasets/adult/datasetfile", feature_remain=None, feature_sample=None,
                          max_feature=0):
    features = {}
    ...
    for idx, k in enumerate(feature_remain):
        ...
        min_corr_mean_index = min(corr_array, key=corr_array.get)
        features[min_corr_mean_index].append(k)

        if len(features[min_corr_mean_index]) >= max_feature:
            value = features.pop(min_corr_mean_index)
            np.save("method/{}/featurefile/shard_{}.npy".format(shard_num, min_corr_mean_index), value)
...
def min_unlearn(Model, Shard_num, Epoch, Opt, Dataset, Batch, feature_first, unlearn_feature):
    ...
    min_corr_distribution(shard_num=Shard_num, dataset=Dataset, feature_remain=feature_remain,
    ...
    with open("GreedyMin.results.txt", "a") as file:
        file.write("dataset---------{}\n".format(Dataset))
...
if __name__ == "__main__":
    ...
    min_unlearn(Model=Model, Shard_num=Shard_num, Epoch=Epoch, Opt=Opt, Dataset=Dataset, Batch=Batch,
